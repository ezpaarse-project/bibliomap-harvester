...
install: ## install depedencies thanks to a dockerized npm install
	@docker run -it --rm -v $$(pwd):/app -w /app --net=host -e NODE_ENV -e http_proxy -e https_proxy node:10.15 npm install
	@make chown
...
chown:
	@test ! -d $$(pwd)/node_modules || docker run -it --rm -v $$(pwd):/app node:10.15 chown -R $$(id -u):$$(id -g) /app/
